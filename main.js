...
function update(timestamp){
    ...
    var sensorDistance=[100,50,20,50,100,50,20,50];
    var sensors=updateSensors(rocket, sensorDistance);
    sensors.push(1);
    var WASD;
    WASD=NN.output(sensors);
    //WASD=getInput();
    rocket.move(WASD);
...
rocket.move=function(WASD){
    if(WASD[0]){
        rocket.vx+=math.cos(rocket.t)*speeds.WSAcceleration;
        rocket.vy+=math.sin(rocket.t)*speeds.WSAcceleration;
    }
    if(WASD[1])rocket.w-=speeds.ADAcceleration;
    if(WASD[2]){
        rocket.vx-=math.cos(rocket.t)*speeds.WSAcceleration;
        rocket.vy-=math.sin(rocket.t)*speeds.WSAcceleration;
    }
    if(WASD[3])rocket.w+=speeds.ADAcceleration;
...
function render(sensors){
    ...
    if(ctx.isPointInPath(sugoMap, rocket.x, rocket.y)){
        ...
        for(var i=0;i<8;i++){
            ...
            ctx.lineTo(rocket.x+sensors[i]*math.cos(rocket.t+sensorDirectories[i]), rocket.y+sensors[i]*math.sin(rocket.t+sensorDirectories[i]));
            ctx.closePath();
            ctx.stroke();
        }
    }else{
