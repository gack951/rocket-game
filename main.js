...
function update(timestamp){
    ...
    checkpoints=[[390, 89], [1030, 95], [1045, 215], [855, 245], [790, 460], [920, 415], [1150, 215], [1260, 330], [490, 645], [530, 390], [410, 285], [110, 365], [15, 250], [90, 120]];
    ...
    if(aliveRocket<=3) return;
    requestAnimationFrame(update);
    aliveRocket=render(rockets, checkpoints);
}
...
function render(rockets, checkpoints){
    // redering
    ...
    for(var r=0;r<rockets.length;r++){
        ...
        aliveRocket++;
        if(ctx.isPointInPath(sugoMap, rockets[r].x, rockets[r].y)){
            ctx.strokeStyle="blue";
            var sensorDirectories=[0, math.pi/4, math.pi/2, math.pi*3/4, math.pi, math.pi*5/4, math.pi*3/2, math.pi*7/4];
            for(var i=0;i<8;i++){
                ctx.beginPath();
                ctx.moveTo(rockets[r].x, rockets[r].y);
                ctx.lineTo(rockets[r].x+rockets[r].sensors[i]*math.cos(rockets[r].t+sensorDirectories[i]), rockets[r].y+rockets[r].sensors[i]*math.sin(rockets[r].t+sensorDirectories[i]));
                ctx.closePath();
                ctx.stroke();
            }
        }else{
        ...
        ctx.closePath();
        ctx.fill();
        ctx.fillText("("+rockets[r].x.toFixed(1)+", "+rockets[r].y.toFixed(1)+", "+rockets[r].t.toFixed(1)+")", 0, 24+12*aliveRocket)
    }
